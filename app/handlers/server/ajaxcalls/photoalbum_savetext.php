<?php
  ...
  if(isParameter("photoalbum_id") && isParameter("photoalbum_name") && isParameter("photoalbum_desc")){
    ...
    try{
      ...
      $album->dbUpdateText();
      $data = $json->encode(array("photoalbum_savetext_success" => lang("Операция успешно завершена")));
    }
    catch(Exception $e){
      $data = $json->encode(array("photoalbum_savetext_error" => $e->getMessage()));
    }
  }
  else{
    $data = $json->encode(array("photoalbum_savetext_error" => lang("Отсутствует необходимый параметр - номер фотоальбома")));
  }
