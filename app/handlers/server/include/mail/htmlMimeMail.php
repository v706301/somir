...
* Last Modified..: $Date: 2009/08/01 07:25:15 $
...
*/
  function buildMessage($params = array())
  {
    ...
    if (isset($message)) {
      ...
      $message_id = sprintf('<%s.%s@%s>', base_convert(time(), 10, 36), base_convert(rand(), 10, 36), !empty($_SERVER["HTTP_HOST"]) ? $_SERVER["HTTP_HOST"] : (!empty($_SERVER["SERVER_NAME"]) ? $_SERVER["SERVER_NAME"] : $_SERVER["HOSTNAME"] ));
