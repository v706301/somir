...
//print("page already added");				
			...
			$this->header_height = $this->GetY() - $this->tMargin;
...
//print("print header");			  
		...
		*/
		protected function checkPageBreak($h) {
			if ((($this->y + $h) > $this->PageBreakTrigger) AND (!$this->InFooter) AND ($this->AcceptPageBreak())) {
				...
				$this->y = $this->tMargin + $this->header_height;				
				$oldpage = $this->page - 1;
				if ($this->rtl) {
    ...
    public function OutputAttachment($name){
      return $this->Output($name,'D');
    }
...
// TODO: Notice: Undefined offset: 26806, if arabic present				  
//print("ta = " . $ta[$i] );
//print("unicode = " . $unicode[$ta[$i]]);
                				  
//					$type = $unicode[$ta[$i]];
//print("[type = " . $type . ", char = " . $ta[$i] . "]" );					
			...
			if ($arabic) {
				...
				for ($i=0; $i < $numchars; ++$i) {
// TODO: Notice: Undefined offset:				  
				    if(isset($unicode[$chardata[$i]['char']])){
...
// TODO: Notice: Undefined offset:					
					if (isset($unicode[$thischar['char']]) && $unicode[$thischar['char']] == 'AL') {
						$x = $thischar['x'];
						if ($x > 0) {
							$prevchar = $charAL[($x-1)];
						} else {
    ...
    function getPageW(){return $this->CurOrientation == 'P' ? $this->fw : $this->fh;}
    function getPageH(){return $this->CurOrientation == 'P' ? $this->fh : $this->fw;}
    function getPrintableW(){return $this->getPageW() - $this->lMargin - $this->rMargin;}
    function getPrintableH(){return $this->getPageH() - $this->tMargin - $this->bMargin;}
    ...
    public function getMarginHeader(){return $this->header_margin;}
    public function getMarginFooter(){return $this->footer_margin;}
    public function getMarginCell(){return $this->cMargin;}
