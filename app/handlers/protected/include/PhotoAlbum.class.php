...
_FIELDS_;
    ...
    public function getExposedCount()
    {
        return $this->exposed_count;
    }

    public function getExposedSize()
    {
        return $this->exposed_size;
    }
    ...
    public function dbReadPhotosRandom($limit = 30)
    {
        global $dbprefix;
        $db = getDatabase();
        $query = sprintf(self::photoFields() . "%s" . "order by rand() limit %d",(User::isOperatorAdmin(User::getOperatorId()) ? "" : "where p.is_hidden=0\n"), $limit);
        //trace($query);
        $photos = array();
        $rs = $db->query($query, __FILE__ . ":" . __LINE__);
...
//      	return false;
    ...
    public static function printSearchByNameControl()
    {
        ...
        <div class="mainFont mainColor L" style="clear:both;padding:5px 0px">
            <div class="bold"
                 style="float:left;margin-right:5px;line-height:22px;vertical-align:middle;"><?php hprint('Поиск по названию')?></div>
            <div style="float:left;"><input onkeypress="if(keyCode(event)==13){submitForm(this.form,'search')}"
                                            type="text" style="width:300px" name="search" value="" class="text"/></div>
            <div style="float:left;">
                <button name="search_btn" type="submit"
                        style="margin:0px 3px;width:22px;height:22px;border:none;background:url(/images/btn-search.gif) no-repeat transparent;"
                        onmouseover="this.style.backgroundPosition='0px -22px'"
                        onmouseout="this.style.backgroundPosition='0px 0px'"
                        onclick="this.style.backgroundPosition='0px -44px';submitForm(this.form,'search');"></button>
            </div>
        </div>
