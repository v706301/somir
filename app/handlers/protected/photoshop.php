<?php
  ...
  $show_menu = false;
  $page_title = lang('Каталог взрослых растений');
...
<?php for($i = ord('A'); $i <= ord('Z'); $i++){?>
        <div onclick="this.style.backgroundPosition='0px -44px';$('#search-field').val('');loadPhotos(this.innerHTML);" class="vmid cursor-ptr bold courier M" onmouseover="this.style.backgroundPosition='0px -22px'" onmouseout="this.style.backgroundPosition='0px 0px'" style="float:left;width:22px;height:22px;line-height:22px;margin:auto 2px auto 0px;background:url(/images/btn-frame.gif) no-repeat;text-align:center;"><?php print(chr($i))?></div>
<?php }?>
      ...
      <div class="mainFont mainColor L" style="clear:both;padding:10px 0px 10px 0px">
        ...
        <div style="float:left;"><input type="text" style="width:300px" id="search-field" name="search" value="<?php hprint($search)?>" class="text"/></div>
